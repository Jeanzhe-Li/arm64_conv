...
int C_Sgemm_op4(float* im2col_weight, float* im2col_feature, float* result_Sgemm, int wh_1, int wh_2, int wh_3) {
    ...
    for (m = 0; m < wh_1; m++) {
        for (n = 0; n < (wh_3 & (~3)); n += 4) {
            // 初始化4个累加器
            result_Sgemm[m*wh_3 + n + 0] = 0;
            result_Sgemm[m*wh_3 + n + 1] = 0;
            result_Sgemm[m*wh_3 + n + 2] = 0;
            result_Sgemm[m*wh_3 + n + 3] = 0;
            
            // 执行1x4的矩阵乘法
            for (k = 0; k < wh_2; k++) {
                result_Sgemm[m*wh_3 + n + 0] += im2col_weight[m*wh_2 + k] * im2col_feature[k*wh_3 + n + 0];
                result_Sgemm[m*wh_3 + n + 1] += im2col_weight[m*wh_2 + k] * im2col_feature[k*wh_3 + n + 1];
                result_Sgemm[m*wh_3 + n + 2] += im2col_weight[m*wh_2 + k] * im2col_feature[k*wh_3 + n + 2];
                result_Sgemm[m*wh_3 + n + 3] += im2col_weight[m*wh_2 + k] * im2col_feature[k*wh_3 + n + 3];
            }
        }
...
int main() {
    ...
    // 开始计时
    printf("开始卷积计算（Im2col + SGEMM 1x4展开）...\n");
    ...
    // 3. 执行矩阵乘法（1x4展开）
    C_Sgemm_op4(weights_data, im2col_feature, output, 
                output_channels, 
